...
from qwen_moe import build_model, EXPERT_LAYER, MoEMLP
import random, time
...
def build_model_nl(nonlinear=False):
    ...
    tok = AutoTokenizer.from_pretrained("qwen_moe.MODEL" if False else "/workspace/models/qwen_ms/models/Qwen--Qwen2.5-0.5B/snapshots/master", use_fast=True)
    if tok.pad_token is None: tok.pad_token = tok.eos_token
    m = AutoModelForCausalLM.from_pretrained("/workspace/models/qwen_ms/models/Qwen--Qwen2.5-0.5B/snapshots/master", use_cache=False, torch_dtype=torch.float32)
