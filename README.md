...
1. **同源初始化**：复制基座第 14 层 FFN 权重，作为各领域专家初始权重
2. **域隔离训练**（`force{i}`）：编程专家只训编程、数学专家只训数学……
...
| 项 | 值 |
|---|---|
| 基座 | Qwen3-0.6B（1024 hidden，28 层，float32，单卡 GTX 1060 6GB） |
| 替换层 | 第 14 层（`EXPERT_LAYER=14`，取中间层） |
| 真实领域语料 | 编程 994 / 数学 3000（源自 `datedsv4f`，经 `prepare_data.py` 提取） |

> 旧实验（Qwen2.5-0.5B / 第 12 层 / CPU）历史结论见 `技术报告_域隔离双专家MoE路由.md`。

## 最新实测结论：真实领域数据（编程 vs 数学）
...
| 编程题 → 编程专家 | 111/149 = 74% |
| 数学题 → 数学专家 | 437/450 = 97% |
| 域隔离训练 | 编程专家 loss 0.838 / 数学专家 loss 2.490 |
| 路由对齐 | 领域监督（已知域标签），80 步，监督 loss 0.184 |

路径：`prepare_data.py`（数据提取/划分）→ `run_real_experiment.py`（GPU 训练 + 路由测试，全程约 1 分钟）。

### 路由对齐的消融（纯 LM loss vs 领域监督 vs 过拟合）
...
| **领域监督** | **80** | **91.5%（最佳）** |
| 领域监督（过拟合） | 120 | 83.5% |
| 领域监督（训练过充分） | 250 | 76.8%（编程专家坍缩） |

> 洞见：路由精度存在**泛化最佳点**——领域监督明显优于纯 LM loss；但监督过强/训练步数过多会把 gate 过拟合到训练分布，导致留出测试上的路由精度反而下降（样本坍缩到单专家）。

## 历史结论（Qwen2.5-0.5B / 多 seed 取平均）
...
| 脚本 | 作用 |
|---|---|
| `prepare_data.py` | **真实数据提取**：从语料源提取编程/数学样本并划分 train/test |
| `run_real_experiment.py` | **最新真实实验**：GPU 域隔离训练 + 路由精度测试（编程 vs 数学） |
...
| `qwen_moe.py` | 核心 MoE 结构（专家同源初始化 / 路由模式 / 输入捕获），基座/替换层配置 |
...
```bash
# 【最新】真实领域实验（编程 vs 数学，GPU）
python3 prepare_data.py            # 提取并划分数据（已入库，仅首次需重跑可跳过）
python3 run_real_experiment.py     # GPU 域隔离训练 + 路由精度测试
